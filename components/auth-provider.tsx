...
type AuthContextType = {
  ...
  updateUserJournal: (entry: JournalEntry) => void
  updateUserMood: (mood: string) => void
  hasRole: (role: "student" | "mentor" | "admin") => boolean
  hasAnyRole: (roles: ("student" | "mentor" | "admin")[]) => boolean
}
...
const createDemoUser = (careerPath = "default"): User => {
  const validCareerPath = (careerPath in careerSkills ? careerPath : "default") as keyof typeof careerSkills
  const validCareerPathCourses = (careerPath in careerCourses ? careerPath : "default") as keyof typeof careerCourses
  const validCareerPathProjects = (careerPath in careerProjects ? careerPath : "default") as keyof typeof careerProjects
  ...
  return {
    ...
    careerPath,
    skills: careerSkills[validCareerPath] || careerSkills.default,
    badges: [
      {
        id: "badge-1",
        name: "Fast Learner",
    ...
    courses: careerCourses[validCareerPathCourses] || careerCourses.default,
    projects: careerProjects[validCareerPathProjects] || careerProjects.default,
...
const setAuthCookies = (user: User, token: string) => {
  ...
  document.cookie = `user-data=${encodeURIComponent(JSON.stringify(user))}; path=/; max-age=${7 * 24 * 60 * 60}; SameSite=Lax`
}
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = async (email: string, password: string) => {
    ...
    try {
      ...
      // Validate credentials (in production, this would be done server-side)
      if (!email || !password) {
      ...
      setAuthCookies(loggedInUser, authToken)
  ...
  const signup = async (name: string, email: string, password: string) => {
    ...
    try {
      ...
      // Create new user with career-specific data
      const newUser = createDemoUser(careerPath)
      newUser.name = name
      newUser.email = email
      ...
      localStorage.setItem("auth-token", authToken)
      setAuthCookies(newUser, authToken)

      toast({
        title: "Account created",
        description: "Welcome to Em-Sphere! Your account has been created successfully.",
      })
  ...
  const demoLogin = async () => {
    ...
    localStorage.setItem("user", JSON.stringify(demoUser))
      localStorage.setItem("auth-token", authToken)
      setAuthCookies(demoUser, authToken)
  ...
  const updateUser = (userData: Partial<User>) => {
    if (user) {
      ...
      // Update cookies
      const token = localStorage.getItem("auth-token") || generateAuthToken()
      setAuthCookies(updatedUser, token)
    }
  }

  const hasRole = (role: "student" | "mentor" | "admin"): boolean => {
    return user?.role === role
  }

  const hasAnyRole = (roles: ("student" | "mentor" | "admin")[]): boolean => {
    return user ? roles.includes(user.role) : false
  }

  const updateUserJournal = (entry: JournalEntry) => {
    if (user) {
      const updatedJournal = [...(user.journalEntries || []), entry]
      updateUser({ journalEntries: updatedJournal })
    }
  }

  const updateUserMood = (mood: string) => {
    if (user) {
      // Update user's mood preference or store in user data
      updateUser({ preferredLearningStyle: mood })
    }
  }
